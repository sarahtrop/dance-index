...
contra_dances.each do |c|
   ...
   figures.each do |f|
      ...
      if c[:a_2].include?(f) && !c[:tag_list].include?(f)
          c[:tag_list].concat(f).concat(', ')
      end
      if c[:b_1].include?(f) && !c[:tag_list].include?(f)
          c[:tag_list].concat(f).concat(', ')
      end
      if c[:b_2].include?(f) && !c[:tag_list].include?(f)
          c[:tag_list].concat(f).concat(', ')
      end
   end
